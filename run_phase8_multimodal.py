...
def prepare_multimodal(n_train=30, n_test=10):
    tr_f, tr_l, te_f, te_l = [], [], [], []
    for d in range(10):
        ...
        for i, idx in enumerate(ch):
            ...
            hog = compute_hog(flat, 16)
            intensity = compute_intensity(flat, 8)
            combined = np.concatenate([hog, intensity])
...
class MultiModalGenome:
    ...
    def random(config, rng, kc_pn_k=6):
        ...
        return MultiModalGenome(config, pn_kc, kc_thresh,
    ...
    def copy(self):
        return MultiModalGenome(self.config, self.pn_kc.copy(), self.kc_thresh.copy(),
...
def mutate(g, rng):
    ...
    c.lr = float(np.clip(c.lr * np.exp(rng.normal(0, 0.2)), 1e-5, 0.01))
    c.reward = float(np.clip(c.reward + rng.normal(0, 0.1), 0.1, 5.0))
    c.punish = float(np.clip(c.punish + rng.normal(0, 0.03), -2.0, -0.01))
    c.w_max = float(np.clip(c.w_max + rng.normal(0, 0.5), 5.0, 50.0))
    c.tau_kc = float(np.clip(c.tau_kc + rng.normal(0, 0.002), 0.005, 0.060))
    c.tau_mbon = float(np.clip(c.tau_mbon + rng.normal(0, 0.002), 0.005, 0.060))
    return c
...
def crossover(a, b, rng):
    ...
    return MultiModalGenome(a.config, pn_kc, kc_thresh,
...
train_feats, train_labs, test_feats, test_labs = prepare_multimodal(30, 10)
...
print(f"Phase 8: Multi-Modal (HOG 128 + Intensity 64 = {n_features} PN)")
print(f"Brain: {brain_cfg.num_neurons} neurons | dt={dt_val}")
...
print("\nJIT warmup...")
wp = [MultiModalGenome.random(brain_cfg, np.random.default_rng(rng.integers(1e9))) for _ in range(2)]
_ = run_eval(wp, brain_cfg, train_spikes[:50], train_labels_full[:50],
             test_spikes[:10], test_labs[:10], tau_m, V_rest, V_reset, g_L)
print("Done.\n")

pop = [MultiModalGenome.random(brain_cfg, np.random.default_rng(rng.integers(1e9))) for _ in range(POP)]
...
for gen in range(GENS):
    ...
    if gen % 5 == 0 or gen == GENS-1:
        ...
        print(f"Gen {gen:3d}: best={h['best']:.4f} mean={h['mean']:.4f} ({h['time']:.0f}s) [{el:.0f}s]")
    si = np.argsort(fit)[::-1]
...
print(f"\n{'='*60}")
print(f"PHASE 8: MULTI-MODAL RESULT")
print(f"Best: {best:.4f}  Time: {total:.0f}s")
print(f"Phase 7 (HOG only, 128 PN): 0.948")
print(f"Phase 8 (HOG+Intensity, {n_features} PN): {best:.4f}")
print(f"{'='*60}")

with open("docs/journal/phase8_multimodal_results.json", "w") as f:
    json.dump({"history": history, "best": best, "time_s": round(total,1),
               "neurons": brain_cfg.num_neurons, "n_features": n_features,
               "modalities": {"hog": 128, "intensity": 64}}, f, indent=2)

fig, axes = plt.subplots(1, 2, figsize=(14, 5))
fig.suptitle("Phase 8: Multi-Modal Integration (HOG + Intensity)", fontsize=14, fontweight="bold")
...
axes[0].axhline(y=0.948, color="green", ls="--", alpha=0.5, label="Phase 7 (0.948)")
axes[0].set_xlabel("Generation"); axes[0].set_ylabel("Fitness")
axes[0].legend(fontsize=8); axes[0].grid(True, alpha=0.3)

comp = {"GA only": 0.268, "STDP": 0.732, "Attention": 0.745, "MultiPass": 0.922,
        "Meta-Evo": 0.948, "Multi-Modal": best}
...
plt.tight_layout()
fig.savefig("docs/figures/phase8_multimodal_results.png", dpi=200, bbox_inches="tight")
plt.close(fig)
print("Figure saved")
