...
@njit(cache=True)
def _simulate_with_eligibility_online(
    W_exc, W_inh, kc_mbon_w, features, V_thresh,
...
def train_and_evaluate_hierarchical(
    pop_W_exc, pop_W_inh, pop_kc_mbon_init, pop_V_thresh,
    pop_train_features, train_labels,
    pop_test_features, test_labels,
    tau_m, V_rest, V_reset, g_L,
    E_exc_val, E_inh_val, tau_exc, tau_inh,
    dt, num_steps, refr_steps,
    ...
    for gi in prange(pop_size):
        np.random.seed(base_seed + gi * 10007)
        kc_mbon = pop_kc_mbon_init[gi].copy()

        for ti in range(n_train):
            mc, ks, elig = _simulate_with_eligibility_online(
                pop_W_exc[gi], pop_W_inh[gi], kc_mbon,
                pop_train_features[gi, ti], pop_V_thresh[gi],
                tau_m, V_rest, V_reset, g_L,
                E_exc_val, E_inh_val, tau_exc, tau_inh,
                dt, num_steps, refr_steps,
                num_neurons, num_pn, kc_start, kc_end, mbon_start, mbon_end,
        ...
        for ti in range(n_test):
            mc, ks = _simulate_forward_online(
                pop_W_exc[gi], pop_W_inh[gi], kc_mbon,
                pop_test_features[gi, ti], pop_V_thresh[gi],
                tau_m, V_rest, V_reset, g_L,
                E_exc_val, E_inh_val, tau_exc, tau_inh,
                dt, num_steps, refr_steps,
                num_neurons, num_pn, kc_start, kc_end, mbon_start, mbon_end,
