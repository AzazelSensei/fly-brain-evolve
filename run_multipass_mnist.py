...
class MultipassGenome:
    ...
    def random(config, rng, kc_pn_k=6):
        ...
        return MultipassGenome(config, pn_kc, kc_thresh,
                               ...
                               max_passes=int(rng.choice([1, 2, 3])),
...
def mutate(g, rng):
    ...
    if rng.random() < 0.1:
        c.max_passes = int(np.clip(c.max_passes + rng.choice([-1, 1]), 1, 3))
    return c
...
print(f"Brain: {brain_cfg.num_neurons} neurons | Phase 5: Multi-pass + Attention + STDP")
...
with open("docs/journal/phase5_multipass_results.json", "w") as f:
...
plt.tight_layout(); fig.savefig("docs/figures/phase5_multipass_results.png", dpi=200, bbox_inches="tight"); plt.close(fig)
print("Figure saved")
