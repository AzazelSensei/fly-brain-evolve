...
def prepare_data(n_train=30, n_test=10):
    ...
    for d in range(10):
        ...
        for i, idx in enumerate(ch):
            ...
            if i < n_train: tr_f.append(hog); tr_l.append(d)
...
class MetaGenome:
    def __init__(self, config, pn_kc, kc_thresh, kc_apl_w=2.0, apl_kc_w=200.0,
                 w_init=5.0, fb_str=0.3, fb_inh=0.4, max_passes=4, conf_thresh=0.1, kc_decay=0.75,
                 lr=0.0003, tau_elig=0.040, tau_kc=0.020, tau_mbon=0.020,
                 reward=1.0, punish=-0.1, w_min=0.0, w_max=15.0):
        self.config = config
        self.pn_kc = pn_kc
    ...
    def random(config, rng, kc_pn_k=6):
        ...
        return MetaGenome(config, pn_kc, kc_thresh,
                          fb_str=rng.uniform(0.1, 0.8), fb_inh=rng.uniform(0.1, 0.6),
                          max_passes=int(rng.choice([2, 3, 4, 5])),
                          conf_thresh=rng.uniform(0.05, 0.5), kc_decay=rng.uniform(0.3, 0.9),
                          lr=10**rng.uniform(-4.5, -2.5),
                          tau_elig=rng.uniform(0.020, 0.080),
                          tau_kc=rng.uniform(0.010, 0.040),
                          tau_mbon=rng.uniform(0.010, 0.040),
                          reward=rng.uniform(0.5, 2.0),
                          punish=rng.uniform(-0.5, -0.02),
                          w_max=rng.uniform(10.0, 25.0))

    def copy(self):
        return MetaGenome(self.config, self.pn_kc.copy(), self.kc_thresh.copy(),
...
def mutate(g, rng):
    ...
    c.kc_decay = float(np.clip(c.kc_decay + rng.normal(0, 0.05), 0.05, 0.95))
    if rng.random() < 0.1:
        c.max_passes = int(np.clip(c.max_passes + rng.choice([-1, 1]), 1, 5))
    c.lr = float(np.clip(c.lr * np.exp(rng.normal(0, 0.3)), 1e-5, 0.01))
    c.tau_elig = float(np.clip(c.tau_elig + rng.normal(0, 0.005), 0.010, 0.100))
    ...
    c.reward = float(np.clip(c.reward + rng.normal(0, 0.1), 0.1, 5.0))
    c.punish = float(np.clip(c.punish + rng.normal(0, 0.05), -2.0, -0.01))
    c.w_max = float(np.clip(c.w_max + rng.normal(0, 1.0), 5.0, 50.0))
    return c
...
def crossover(a, b, rng):
    ...
    return MetaGenome(a.config, pn_kc, kc_thresh,
                      ...
                      tau_elig=pick(a.tau_elig, b.tau_elig),
                      tau_kc=pick(a.tau_kc, b.tau_kc),
                      tau_mbon=pick(a.tau_mbon, b.tau_mbon),
                      reward=pick(a.reward, b.reward),
                      punish=pick(a.punish, b.punish),
...
print(f"Brain: {brain_cfg.num_neurons} neurons | dt={dt_val} | Phase 7: Meta-Evolution")
...
print("\nJIT warmup...")
wp = [MetaGenome.random(brain_cfg, np.random.default_rng(rng.integers(1e9))) for _ in range(2)]
_ = run_eval(wp, brain_cfg, train_spikes[:50], train_labels_full[:50],
             test_spikes[:10], test_labs[:10], tau_m, V_rest, V_reset, g_L)
print("Done.\n")

print(f"{'='*70}")
print(f"PHASE 7: META-EVOLUTION (Learning to Learn)")
print(f"STDP params (lr, tau, reward, punish, w_max) are NOW in the genome")
print(f"POP={POP}, GENS={GENS}")
print(f"{'='*70}\n")

pop = [MetaGenome.random(brain_cfg, np.random.default_rng(rng.integers(1e9))) for _ in range(POP)]
history = []; t0_total = time.time()

for gen in range(GENS):
    t0 = time.time()
    fit = run_eval(pop, brain_cfg, train_spikes, train_labels_full,
    ...
    h = {"gen": gen, "best": round(float(fit[bi]),4), "mean": round(float(fit.mean()),4),
         ...
         "lr": f"{bg.lr:.5f}", "tau_e": f"{bg.tau_elig:.3f}",
         "reward": f"{bg.reward:.2f}", "punish": f"{bg.punish:.2f}",
         "passes": bg.max_passes, "w_max": f"{bg.w_max:.1f}"}
    history.append(h)
    if gen % 5 == 0 or gen == GENS-1:
        el = time.time()-t0_total
        print(f"Gen {gen:3d}: best={h['best']:.4f} mean={h['mean']:.4f} "
              f"lr={bg.lr:.5f} rw={bg.reward:.1f} pu={bg.punish:.2f} "
              f"p={bg.max_passes} ({h['time']:.0f}s) [{el:.0f}s]")
    si = np.argsort(fit)[::-1]
...
bg = pop[np.argmax(fit)]

print(f"\n{'='*70}")
print(f"PHASE 7: META-EVOLUTION RESULT")
print(f"Best: {best:.4f}  Time: {total:.0f}s")
print(f"Phase 5 tuned (fixed params): 0.922")
print(f"Evolved STDP params:")
print(f"  lr={bg.lr:.6f}  tau_elig={bg.tau_elig:.3f}  tau_kc={bg.tau_kc:.3f}  tau_mbon={bg.tau_mbon:.3f}")
print(f"  reward={bg.reward:.3f}  punish={bg.punish:.3f}  w_max={bg.w_max:.1f}")
print(f"  passes={bg.max_passes}  conf={bg.conf_thresh:.2f}  decay={bg.kc_decay:.2f}")
print(f"{'='*70}")

with open("docs/journal/phase7_meta_results.json", "w") as f:
    json.dump({"history": history, "best": best, "time_s": round(total,1),
               "best_params": {"lr": bg.lr, "tau_elig": bg.tau_elig, "tau_kc": bg.tau_kc,
                               "tau_mbon": bg.tau_mbon, "reward": bg.reward, "punish": bg.punish,
                               "w_max": bg.w_max, "passes": bg.max_passes,
                               "conf": bg.conf_thresh, "decay": bg.kc_decay}}, f, indent=2)

fig, axes = plt.subplots(1, 3, figsize=(18, 5))
fig.suptitle("Phase 7: Meta-Evolution — Learning to Learn", fontsize=14, fontweight="bold")
...
axes[0].axhline(y=0.922, color="green", ls="--", alpha=0.5, label="Phase 5 (0.922)")
axes[0].axhline(y=0.1, color="gray", ls="--", alpha=0.5)
axes[0].set_xlabel("Generation"); axes[0].set_ylabel("Fitness")
axes[0].legend(fontsize=8); axes[0].grid(True, alpha=0.3)
axes[0].set_title("Fitness Over Generations")

comp = {"GA only": 0.268, "STDP v3": 0.732, "Attention": 0.745,
        "MultiPass": 0.890, "Tuned": 0.922, "Meta-Evo": best}
colors = ["#E74C3C"]*5 + ["#2ECC71"]
axes[1].barh(list(comp.keys()), list(comp.values()), color=colors, alpha=0.7, edgecolor="black")
axes[1].axvline(x=0.1, color="gray", ls="--", alpha=0.5)
axes[1].set_xlabel("Best Fitness"); axes[1].set_title("All Phases")
for i, v in enumerate(comp.values()):
    axes[1].text(v+0.005, i, f"{v:.3f}", va="center", fontsize=9)

params_names = ["lr", "tau_elig", "reward", "punish", "w_max"]
params_vals = [bg.lr*1000, bg.tau_elig*1000, bg.reward, abs(bg.punish), bg.w_max/10]
axes[2].bar(params_names, params_vals, color="#9B59B6", alpha=0.7, edgecolor="black")
axes[2].set_title("Evolved STDP Parameters (normalized)")
axes[2].set_ylabel("Value"); axes[2].grid(True, alpha=0.3, axis="y")

plt.tight_layout()
fig.savefig("docs/figures/phase7_meta_results.png", dpi=200, bbox_inches="tight")
plt.close(fig)
print("Figure saved")
